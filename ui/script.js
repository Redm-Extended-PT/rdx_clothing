...
document.getElementById("eyewear").value = event.data.okulary;
  ...
  $('input').change(function(){	  
    $.post('http://rdx_clothing/updateClothes', JSON.stringify({
      value: false,
// Body    
      hat: $('.hat').val(),
	  shirt: $('.shirt').val(),
...
// Body
  ...
  $('.yes').on('click', function(e){
    e.preventDefault();
    $.post('http://rdx_clothing/saveClothes', JSON.stringify({
      value: true,
      hat: $('.hat').val(),
	  shirt: $('.shirt').val(),
	  vest: $('.vest').val(),
    ...
    e.preventDefault();
    $.post('http://rdx_clothing/exit');
  }); 
  
  // Rotate player
  $(document).keypress(function(e) {
    if(e.which == 113){ // Q pressed
      $.post('http://rdx_clothing/heading', JSON.stringify({
        value: -10
      }));
    }
  if(e.which == 100){ // D pressed
      $.post('http://rdx_clothing/defcam');
    }
