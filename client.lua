...
RegisterNetEvent('rdx_clothing:sex')
AddEventHandler('rdx_clothing:sex', function(skin, ubranie)
...
RegisterNUICallback('exit', function()
    TriggerEvent("rdx_skin:FastLoad")
...
RegisterNUICallback('saveClothes', function(data, cb)
    ...
    if data.name ~= nil and data.name ~= "" then
        saveOutfit = true
        TriggerServerEvent("rdx_clothing:Save", json, price, saveOutfit , data.name, function(cb)
            if cb then
                print("DONE")
            else
                print("ERROR")
        ...
        TriggerServerEvent("rdx_clothing:Save", json, price, saveOutfit ,function(cb)
            if cb then
                print("DONE")
            else
                print("ERROR")
...
RegisterNetEvent('rdx_clothing:load2')
AddEventHandler('rdx_clothing:load2', function()
    TriggerServerEvent("rdx_clothing:loadClothes", 1, function(cb)
        end)
    fast = true
end)
...
RegisterNetEvent('rdx_clothing:updateClothes')
AddEventHandler('rdx_clothing:updateClothes', function(data)
...
Citizen.CreateThread(function()
    ...
    if sex == 1 then
		...
		Citizen.InvokeNative(0xCC8CA3E88256E58F, _target, 0, 1, 1, 1, 0) -- Actually remove the component
		 TriggerServerEvent('rdx_respawn:TestStatus')
		if _t ~= nil then
...
local Outfits_tab = {}
RegisterNetEvent('rdx_clothing:OpenOutfits')
AddEventHandler('rdx_clothing:OpenOutfits', function()
    TriggerServerEvent('rdx_clothing:getOutfits')
end)

RegisterNetEvent('rdx_clothing:putInTable')
AddEventHandler('rdx_clothing:putInTable', function(outfit)
    Outfits_tab = {}
...
MenuData = {}
TriggerEvent("rdx_menu_base:getData",function(call)
    MenuData = call
end)
function Outfits()
...
function OutfitsManage(outfit)
    ...
    MenuData.Open(
        ...
        function(data, menu)
            menu.close()
            TriggerServerEvent('rdx_clothing:'..data.current.value , outfit)
        end,
        ...
        for k,v in pairs(Config.Zones) do
            ...
            if dist < 2 then
                ...
                if IsControlJustReleased(0, Config.OpenKey) then
                    TriggerServerEvent("rdx_clothing:loadClothes", 2, function(cb)
                        end)
                end
            else
